...
export default function BestDeals() {
  ...
  useEffect(() => {
    productService?.getProducts
      ? productService
          .getProducts({ sortBy: "price-low", limit: 6 })
          .then((data) => {
    ...
    <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <h2
            className="text-xl md:text-3xl font-extrabold text-slate-900"
        ...
        <Link
          to="/all-products"
          className="flex items-center gap-1 text-sm md:text-base font-semibold text-indigo-600 hover:text-indigo-700 transition"
        >
      ...
      <div className="rounded-3xl bg-white border border-indigo-100 p-5 sm:p-6 shadow-sm">
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {deals.map((deal, index) => {
            const prodId = deal._id || deal.id || `bk-deal-${index + 1}`;
            return (
              <Link
              ...
              >
                <div className="h-full p-4 rounded-2xl bg-[var(--cm-bg)] border border-slate-100 group-hover:border-indigo-200 group-hover:shadow-md transition-all duration-300 flex flex-col justify-between">
                  ...
                  <div className="flex items-center justify-between mt-3 pt-2 border-t border-slate-200/60">
                    <p className="text-lg text-emerald-600 font-extrabold">
